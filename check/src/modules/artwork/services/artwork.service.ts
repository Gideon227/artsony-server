...
export async function getFeed(
  ...
  switch (mode) {
    ...
    case 'for_you': {
      ...
      //
      // If the requester explicitly picked categories via the filter bar,
      // that always wins — this heuristic only fills in a default when they
      // haven't chosen anything themselves.
      ...
      const engagedCategories = await artworkRepository.getEngagedCategories(requesterId)
      if (engagedCategories.length === 0) {
        return artworkRepository.list({ ...base, sort_by: 'like_count', sort_order: 'desc' })
      }
      return artworkRepository.list({ ...base, categories: engagedCategories, sort_by: 'created_at', sort_order: 'desc' })
    }
  }
}
