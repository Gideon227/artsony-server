...
const wallet_repository_1 = require("../../../modules/wallet/repositories/wallet.repository");
const review_repository_1 = require("../../../modules/review/repositories/review.repository");
const analytics_types_1 = require("../../../common/types/analytics.types");
...
function periodWindows(period, now = new Date()) {
    const days = analytics_types_1.PERIOD_DAYS[period];
...
function scoreBuyerSatisfaction(averageRating) {
    if (averageRating === null)
        return COLD_START_SATISFACTION_SCORE;
    return clamp(((averageRating - 1) / 4) * 100, 0, 100);
}
function scoreEngagement(views, likes) {
    if (views < ENGAGEMENT_MIN_VIEWS_FOR_SIGNAL)
        return COLD_START_ENGAGEMENT_SCORE;
...
function scoreOrderReliability(input) {
    ...
    if (input.totalPhysical === 0) {
        const subRatings = [input.avgConditionRating, input.avgDeliveryRating].filter((v) => v !== null);
...
exports.analyticsService = {
    // ── Overview ───────────────────────────────────────────────────────────────
    async getOverview(sellerId, period = 'month') {
        ...
        const [current, previous, balance] = await Promise.all([
            analytics_repository_1.analyticsRepository.getPeriodMetrics(sellerId, currentStart, currentEnd),
            analytics_repository_1.analyticsRepository.getPeriodMetrics(sellerId, previousStart, previousEnd),
            wallet_repository_1.walletRepository.getBalanceSummary(sellerId),
        ]);
        return {
    ...
    async getDailyEarnings(sellerId, year) {
        ...
        if (year < 2020 || year > currentYear) {
            throw new errors_1.ValidationError('Validation failed', { year: `year must be between 2020 and ${currentYear}` });
        }
        return analytics_repository_1.analyticsRepository.getDailyEarnings(sellerId, year);
    },
    // ── Sales feed ─────────────────────────────────────────────────────────────
    async getSalesAnalytics(sellerId, filters) {
        if (filters.price_min !== undefined && filters.price_max !== undefined && filters.price_min > filters.price_max) {
            throw new errors_1.ValidationError('Validation failed', { price_min: 'price_min must not exceed price_max' });
        }
        return analytics_repository_1.analyticsRepository.getSalesAnalytics(sellerId, filters);
    },
    // ── Top artworks ───────────────────────────────────────────────────────────
    async getTopArtworks(sellerId, period = 'week', metric = 'earnings', limit = 5) {
        const { currentStart, currentEnd, previousStart, previousEnd } = periodWindows(period);
        const [current, previous] = await Promise.all([
            analytics_repository_1.analyticsRepository.getArtworkPerformance(sellerId, currentStart, currentEnd),
            analytics_repository_1.analyticsRepository.getArtworkPerformance(sellerId, previousStart, previousEnd),
        ]);
        ...
        const metricValue = (item) => metric === 'earnings' ? item.earnings.current
    ...
    // ── Artsony Score ──────────────────────────────────────────────────────────
    async getArtsonyScore(sellerId) {
        const now = new Date();
        ...
        const [ratingStats, reliability, engagementMetrics] = await Promise.all([
            review_repository_1.reviewRepository.getSellerRatingStats(sellerId),
            analytics_repository_1.analyticsRepository.getReliabilityStats(sellerId),
            analytics_repository_1.analyticsRepository.getPeriodMetrics(sellerId, engagementStart, now),
        ]);
        ...
        const composite = buyerSatisfactionScore * SCORE_WEIGHTS.buyerSatisfaction +
