...
exports.walletRepository = {
    ...
    async getBalanceSummary(userId) {
        const result = await (0, database_1.supabase)().rpc('get_artist_balance_summary', {
            p_user_id: userId,
        });
        if (result.error) {
    ...
    async listLedger(userId, filters) {
        ...
        if (filters.hold_status)
            query = query.eq('hold_status', filters.hold_status);
    ...
    async requestWithdrawal(input) {
        const result = await (0, database_1.supabase)().rpc('request_withdrawal', {
        ...
        const row = (result.data ?? [])[0];
        if (!row)
            throw new Error('[Supabase:wallet.requestWithdrawal] No row returned');
        return toWithdrawal(row);
    },
    // ── TransitionWithdrawal ──────────────────────────────────────────────────────
    async transitionWithdrawal(input) {
        const result = await (0, database_1.supabase)().rpc('transition_withdrawal', {
        ...
        const row = (result.data ?? [])[0];
        if (!row)
            throw new Error('[Supabase:wallet.transitionWithdrawal] No row returned');
        return toWithdrawal(row);
    },
    // ── FindWithdrawalById ────────────────────────────────────────────────────────
    async findWithdrawalById(id) {
        ...
        if (result.error?.code === 'PGRST116')
            return undefined;
        (0, database_1.assertNoError)(result, 'wallet.findWithdrawalById');
