...
const followUserEmbed = (fkConstraint) => `
  id,
  users!${fkConstraint} (
    id,
    username,
...
exports.followRepository = {
    ...
    async toggle(followerId, followingId) {
        const result = await (0, database_1.supabase)().rpc('toggle_follow', {
    ...
    async getFollowedIds(followerId) {
        ...
        return (result.data ?? []).map((r) => r['following_id']);
    },
    ...
    async listFollowers(userId, filters) {
        ...
        return {
            data: (result.data ?? []).map((r) => toFollowUser(r.users)),
