...
import { PLATFORM_SERVICE_FEE_RATE } from '@/common/types/commerce.types'
...
function buildPdfBuffer(data: InvoiceData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.text(
      `Issue Date: ${data.purchase_date.toLocaleDateString('en-GB', {
    ...
    for (const item of data.items) {
      ...
      doc.fontSize(9).fillColor('#25282D')
        ...
        .text(
          `${data.currency} ${item.shipping_cost.toFixed(2)}`,
          420, rowY, { width: 65, lineBreak: false },
        )
        .text(
          `${data.currency} ${item.unit_price.toFixed(2)}`,
          490, rowY, { width: 55, align: 'right', lineBreak: false },
        )
    ...
    // ── Totals ────────────────────────────────────────────────────────────────
    const platformFee = subtotal * PLATFORM_SERVICE_FEE_RATE
    ...
    doc.text(
      `Platform fee (${(PLATFORM_SERVICE_FEE_RATE * 100).toFixed(0)}%):`,
      labelX, totY + lineH * 2,
    )
    ...
    if (data.refund_status !== 'NONE') {
      doc.fontSize(8).fillColor('#788191').text(
        `Note: Refund status — ${data.refund_status}. Shipping charges are non-refundable.`,
        50, totY + lineH * 6, { width: 495 },
      )
    }
...
export const invoiceService = {
  ...
  async generate(input: {
    ...
    const latest      = await physicalOrderRepository.getLatestInvoice(input.order_id)
    ...
    return physicalOrderRepository.upsertInvoice({
  ...
  async getLatest(orderId: string): Promise<OrderInvoice | null> {
    return physicalOrderRepository.getLatestInvoice(orderId)
  },
}

// ═══════════════════════════════════════════════════════════════════════════
// Receipt — proof of payment. Distinct document from the invoice above:
// a receipt confirms payment was received (amount, method, transaction
// reference) and is issued exactly once per order. An invoice itemizes
// the goods/services, agreed prices, and payment terms, and may be
// regenerated. These are intentionally separate PDFs with separate
// storage rows (order_receipts vs order_invoices).
// ═══════════════════════════════════════════════════════════════════════════
...
function buildReceiptPdfBuffer(data: ReceiptData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc    = new PDFDocument({ size: 'A4', margin: 50 })
    const chunks: Buffer[] = []
    ...
    doc.text(`Order ID: ${data.order_id}`,               50, 124)
    doc.text(
      `Payment Date: ${data.payment_date.toLocaleDateString('en-GB', {
        day: '2-digit', month: 'long', year: 'numeric',
      })}`,
      50, 138,
    )
    ...
    doc.fontSize(8).fillColor('#788191').text(
      'This receipt confirms payment was received for the above order. ' +
      'For an itemized breakdown of goods, prices, and shipping, refer to the order invoice.',
      50, boxTop + 102, { width: 495 },
    )
...
export const receiptService = {
  ...
  async generate(input: {
    ...
    const existing = await physicalOrderRepository.getReceipt(input.order_id)
    ...
    return physicalOrderRepository.createReceipt({
      order_id:                 input.order_id,
      pdf_cloudinary_public_id: uploaded.public_id,
      pdf_url:                  uploaded.secure_url,
  ...
  async getLatest(orderId: string): Promise<OrderReceipt | null> {
    return physicalOrderRepository.getReceipt(orderId)
  },
}