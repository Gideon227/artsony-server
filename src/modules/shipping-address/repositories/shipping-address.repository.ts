...
function toShippingAddress(row: any): ShippingAddress {
  return {
    id:              row['id'],
    user_id:         row['user_id'],
...
export const shippingAddressRepository = {
  ...
  async listByUser(userId: string): Promise<ShippingAddress[]> {
    ...
    assertNoErrorMany(result, 'shippingAddress.listByUser')
  ...
  async findById(id: string, userId: string): Promise<ShippingAddress | undefined> {
    const result = await (supabase() as any)
      .from('shipping_addresses')
      .select('*')
      .eq('id', id)
    ...
    assertNoError(result, 'shippingAddress.findById')
  ...
  async create(userId: string, input: CreateShippingAddressInput): Promise<ShippingAddress> {
    ...
    assertNoError(result, 'shippingAddress.create')
    return toShippingAddress(result.data)
  },
  ...
  async setDefault(id: string, userId: string): Promise<ShippingAddress | undefined> {
    const result = await (supabase() as any).rpc('set_default_shipping_address', {
      p_user_id:    userId,
      p_address_id: id,
    })

    if (result.error) {
      if (result.error.code === 'P0002') return undefined
      throw Object.assign(
        new Error(`[Supabase:shippingAddress.setDefault] ${result.error.message}`),
